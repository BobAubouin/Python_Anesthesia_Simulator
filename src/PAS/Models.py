...
class PKmodel:
    ...
    def __init__(self, age: float, height: float,
        ...
        self.A_d, self.B_d, self.C_d, self.D_d = discretize(
            self.A, self.B, self.C, self.D, self.Te, method="exact")
        # init outout
        self.x = x0
    ...
    def update_coeff_CO(self, CO: float, CO_init: float = 6.5):
        ...
        self.A_d, self.B_d, self.C_d, self.D_d = discretize(
            Anew, self.B, self.C, self.D, self.Te, method="exact")
...
class AtraciumPKPD:
    ...
    def compute_bis(self):
        effect = 100 * self.c50 ** self.gamma / \
            (self.ce ** self.gamma + self.c50 ** self.gamma)
        return effect
