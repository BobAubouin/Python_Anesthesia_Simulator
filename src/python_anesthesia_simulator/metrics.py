...
def compute_control_metrics(Bis: list, Ts: float = 1, phase: str = 'maintenance', latex_output: bool = False):
    ...
    if phase == 'induction':
        ...
        second_step_time = 15
        BIS_NADIRp = min(Bis[:int(second_step_time*60/Ts)])
        BIS_NADIRn = max(Bis[int(second_step_time*60/Ts):])
        TTp, TTn = np.nan, np.nan
        for j in range(int(60/Ts)+1, int(second_step_time*60/Ts)):
            if Bis[j] < 55:
                TTp = (j*Ts-60)/60
                break

        for j in range(int(second_step_time*60/Ts)+1, len(Bis)):
            if Bis[j] < 45:
                TTn = (j*Ts-5*60)/60
                break
