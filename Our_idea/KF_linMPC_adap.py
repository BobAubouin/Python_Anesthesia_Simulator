...
def simu(Patient_info: list,style: str, MPC_param: list, EKF_param: list, random_PK: bool = False, random_PD: bool = False):
    ...
    if False:
        ...
        estimator.init_first_step(BIS_cible)
        plt.plot(np.arange(0,len(X_output[0,:])), np.ones(len(X_output[0,:]))*estimator.a,  'r--', label='true a')
        plt.plot(np.arange(0,len(X_output[0,:])), np.ones(len(X_output[0,:]))*estimator.b,  'b--', label='true b')
        plt.plot(np.arange(0,len(X_output[0,:])), np.ones(len(X_output[0,:]))*estimator.c,  'g--', label='true c')
        plt.legend()
        plt.show()
...
for i in range(8,9):
    ...
    p1.line(np.arange(0,len(data[0]))/60, data[5], legend_label='internal target', line_color="#f46d43")
