...
from src.Control.Estimators import linear_Kalman, EKF_extended
...
def simu(Patient_info: list,style: str, MPC_param: list, EKF_param: list, random_PK: bool = False, random_PD: bool = False):
    ...
    Q = Q_continuous_white_noise(4, spectral_density = 10**(-3), block_size = 2)
    Q = block_diag(Q, Q_continuous_white_noise(3, spectral_density = 1e10, block_size = 1))
    P0 = np.diag([1]*8 + [1e3]*3)*10**EKF_param[1]
    estimator = EKF_extended(A_nom, B_nom, BIS_param = BIS_param_nominal, ts = ts,
                    P0 = P0, R = 10**0, Q = Q)
    ...
    if style=='induction':
        ...
        for i in range(N_simu):
            ...
            Xr_EKF[:,i] =  X[4:8]
            # X_output[:,i] = X[8:]
            if (BIS[i]<70 and BIS[i]>30) :
                Cep_good.append(Xp_EKF[3,i])
                Cer_good.append(Xr_EKF[3,i])
                Bis_good.append(BIS[i])
                
            if i%20==0 and len(Cep_good)>40:
            ...
            if  i==100/ts : #or (BIS_EKF[i]<50 and MPC_controller.ki==0):
                MPC_controller.ki = ki_mpc
            X = np.clip(X[:8], a_min=0, a_max = 1e10)
            if i%ts_MPC==0:
                uP, uR = MPC_controller.one_step(X, BIS_cible, BIS[i])
    ...
    if True:
        ...
        plt.figure()
        plt.plot(np.arange(0,len(X_output[0,:]))*ts/60, X_output[0,:], 'r', label='a')
        plt.plot(np.arange(0,len(X_output[0,:]))*ts/60, X_output[1,:], 'b', label='b')
        plt.plot(np.arange(0,len(X_output[0,:]))*ts/60, X_output[2,:], 'g', label='c')
        estimator = EKF_extended(A_nom, B_nom, BIS_param = BIS_param, ts = ts,
                        P0 = P0, R = 10**EKF_param[2], Q = Q)
        estimator.init_first_step(BIS_cible)
        plt.plot(np.arange(0,len(X_output[0,:]))*ts/60, np.ones(len(X_output[0,:]))*estimator.a,  'r--', label='true a')
        plt.plot(np.arange(0,len(X_output[0,:]))*ts/60, np.ones(len(X_output[0,:]))*estimator.b,  'b--', label='true b')
        plt.plot(np.arange(0,len(X_output[0,:]))*ts/60, np.ones(len(X_output[0,:]))*estimator.c,  'g--', label='true c')
        plt.legend()
        plt.show()
        ...
        ax.plot3D(Xp_EKF[3,:], Xr_EKF[3,:], BIS)
        plt.show()
